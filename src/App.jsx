...
class App extends React.Component {
  ...
  render() {
    const menus = routes.filter(item => item.noMenu !== 2).map((item) => {
      if (!!item.children && item.children.length > 0) {
        ...
        return (
          ...
          >
            { childrenRoutes.filter(item3 => item3.noMenu !== 2).map((item2) => {
              return (
                <Item key={(!!item.key ? item.key : item.path) + (!!item2.key ? item2.key : item2.path)}>{item2.name}</Item>
              );
            }) }
          </SubMenu>
        );
